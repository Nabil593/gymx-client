...
function LoginForm() {
    ...
    return (
        <div className="w-full max-w-[400px] bg-zinc-950 border border-zinc-800 rounded-xl p-6 sm:p-8 shadow-2xl">
            <div className="flex flex-col space-y-1.5 text-center mb-6">
                <h1 className="text-xl font-extrabold tracking-tight uppercase text-zinc-100">Sign In to GymX</h1>
                <p className="text-xs text-zinc-400">Enter your credentials to auto-route to your dedicated panel</p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <div className="space-y-1.5 text-left">
                    ...
                    <input type="email" placeholder="admin@ironpulse.com" className="w-full h-9 bg-black text-xs px-3 rounded border border-zinc-800 focus:border-zinc-700 text-zinc-200 focus:outline-none transition-colors" {...register("email", { required: "Email is required" })} />
                ...
                <button type="submit" disabled={isLoading} className="w-full bg-zinc-50 hover:bg-zinc-200 text-black h-9 rounded text-xs font-bold flex items-center justify-center transition-colors mt-2 cursor-pointer disabled:opacity-50">
            ...
            <button type="button" onClick={handleGoogleSignIn} disabled={isLoading} className="w-full bg-black hover:bg-zinc-900 border border-zinc-800 text-zinc-200 h-9 rounded text-xs font-semibold flex items-center justify-center transition-colors gap-2 cursor-pointer disabled:opacity-50">
            ...
            <p className="mt-6 text-center text-xs text-zinc-500">
                New to GymX?{" "}
                <Link href="/register" className="text-zinc-300 hover:text-zinc-50 underline font-medium">
                    Create an account
                </Link>
            </p>
        </div>
...
export default function LoginPage() {
    return (
        <div className="w-full min-h-[75vh] flex items-center justify-center bg-black text-zinc-50 px-4 py-12">
            {/* Suspense বাউন্ডারি ব্যবহার করা হয়েছে যাতে বিল্ড এরর না হয় */}
