    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            try {
                ...
                if (userEmail) {
                    const bookingRes = await fetch(`${baseUrl}/api/check-booked?email=${encodeURIComponent(userEmail)}&classId=${id}`);
                    const bookingData = await bookingRes.json();
                    setIsBooked(!!(bookingData.isBooked || bookingData.hasBooked));

                    const favRes = await fetch(`${baseUrl}/api/check-favorite?email=${encodeURIComponent(userEmail)}&classId=${id}`);
